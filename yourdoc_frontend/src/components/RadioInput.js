export function RadioInput({ options = [], label, name, onChange }) {
  return <>
    ...
    <ul class="grid w-full gap-6 md:grid-cols-2">
      {options.map(o => <li onClick={() => onChange(o.value)}>
...
export function RadioInput2({ options = [], label, name, onChange, isRequired }) {
  return <>
    ...
    <ul class="items-center w-full text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg sm:flex dark:bg-gray-700 dark:border-gray-600 dark:text-white">
      {options.map(o => <li onClick={() => onChange?.(o.value)} class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600">
